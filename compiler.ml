...
let new_label =
  ...
  fun () ->
    ...
    let rec compile_expr e env info =
      ...
      | String s ->(*J'ai du rajouter une condition pour différencier les chaîne de caractère de variable et de string par rapport au guillement,*)
    ...
    let rec compile_instr instr info =
      ...
      | Print p ->(*J'ai essayé de fair een sorte que le print soit faisaible sur une nombre de valeur dynamique au lieu de faire un print pour chaque type dans baselib*)
              (*ça évite aussi de surchargé le parser de plein de définition de print pour chaque type*)
              (*j'ai voulu faire le même fonctionnement pour scanf, mais j'ai pas réussi*)
              ...
              (new_info, acc_asm @ compiled_expr @ match e with
                ...
                | _ -> failwith "print can only be used with int or string") (*Il faudrait que je gère le cas Bool, peut être call aussi mais je ne vois pas l'intérêt d'afficher des appels de fonctions*)
            ) p (info, [])
            in
